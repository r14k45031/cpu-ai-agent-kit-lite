...
- Word **就地 AI 改寫**：保留樣式、表格、圖片
- Excel AI 分析報告與資料問答；文件一鍵變 PowerPoint 簡報
- Excel **AI 欄位處理**：逐列判斷／分類／翻譯，結果寫進新欄位
- 右鍵「傳送到」選單整合＋**資料夾監看全自動處理**
- 文件一鍵**轉 Markdown**（Word/PDF/PPT/Excel → .md，整理資料、餵知識庫）
...
| 拖入Word就地AI改寫.bat | 拖 docx 上去，逐段改寫、版面全保留 | `_就地改寫.docx` |
| 拖入Excel分析.bat | 拖 xlsx/csv 上去，問資料問題（Enter＝分析報告） | `_分析.docx` |
| 拖入文件變簡報.bat | 拖文件上去，AI 整理大綱產生簡報 | `_簡報.pptx` |
| 拖入Excel欄位AI處理.bat | 拖 xlsx/csv 上去，AI 逐列處理寫進新欄位 | `_AI處理.xlsx` |
| 啟動資料夾監看.bat | 檔案丟進收件匣就自動摘要/OCR | 「完成」資料夾 |
| 拖入文件轉MD.bat | 拖檔案或**整個資料夾**上去（不經 AI，速度快） | 同名 `.md` |
| 拖入MD轉Word.bat | 拖 AI 產出的 .md/.txt 上去 | 同名 `.docx` |

- 摘要工具遇到沒有文字層的掃描 PDF 會自動先 OCR 再摘要；圖片檔（PNG/JPG）也能直接拖入。
- 想調整提示詞，直接編輯 `tools/scripts/` 裡的 Python 檔。

### 能力邊界（誠實聲明）

- **能做**：讀取 Word／Excel／PowerPoint／PDF 內容；「就地AI改寫」逐段改寫並**保留段落樣式、標題階層、表格、圖片**；Excel AI 分析；文件一鍵變簡報。
- **僅有的限制**：被改寫段落「內部」的字元級格式（如句中幾個字加粗）會統一成該段的第一種格式；圖表與公式不會重算。
